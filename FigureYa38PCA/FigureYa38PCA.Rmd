...
tryCatch({
  ...
  cat("Successfully loaded ggord.R\n")
}, error = function(e) {
  cat("Error loading ggord.R:", e$message, "\n")
  stop("Failed to load ggord.R")
})

tryCatch({
  source("geom_ord_ellipse.R") # from gglayer package, https://github.com/GuangchuangYu/gglayer
  cat("Successfully loaded geom_ord_ellipse.R\n")
}, error = function(e) {
  cat("Error loading geom_ord_ellipse.R:", e$message, "\n")
  stop("Failed to load geom_ord_ellipse.R")
})

# 检查关键函数是否存在
# Check if key functions exist
if (!exists("ggord")) {
  stop("ggord function not found after loading ggord.R")
}
if (!exists("StatOrdEllipse")) {
  stop("StatOrdEllipse not found after loading geom_ord_ellipse.R")
}
...
用箭头标出对 PCA 贡献最大的 N 个（这里以 3 为例）基因，把抽象的降维结果转化为可解释的生物学故事。

Mark the top N (3 in this example) genes with arrows that contribute the most to PCA, transforming abstract dimensionality reduction results into interpretable biological stories.
...
- 可以用箭头标出对PCA贡献最大的N个基因，而不是全部基因
- You can use arrows to mark the top N genes that contribute to PCA, rather than all genes
